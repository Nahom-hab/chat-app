        ...
        <ul className="space-y-2 mt-4  ml-3 lg:block overflow-y-auto max-h-[34rem] scrollbar-light dark:scrollbar-dark">
            {friends && friends?.map((friend) => (
                ...
                >
                    ...
                    />
                    {onlineUsers.includes(friend._id) ? <div className="w-3 h-3 relative bottom-3  right-5 rounded-full bg-green-500"></div> : ''}
                    <span className={`text-gray-900 ${onlineUsers.includes(friend._id) ? 'relative right-[13px]' : ''} dark:text-white`}>{friend.full_name.length < 10 ? friend.full_name : (`${friend.full_name.slice(0, 10)}...`)}</span>
                </li>
            ))}
        </ul>
